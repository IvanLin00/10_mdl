  ...
  for (i=0;i<lastop;i++) {
    ...
    switch (op[i].opcode)
    {
      ...
      case SPHERE:
        ...
        else draw_polygons(tmp,t,zb,view,light,ambient,reflect);
      ...
      case ROTATE:
        ...
        double angle = op[i].op.rotate.degrees * (M_PI/180);
        if (op[i].op.rotate.axis == 0) tmp = make_rotX(angle);
        if (op[i].op.rotate.axis == 1) tmp = make_rotY(angle);
        if (op[i].op.rotate.axis == 2) tmp = make_rotZ(angle);
        matrix_mult(peek(systems),tmp);
        copy_matrix(tmp,peek(systems));
        tmp->lastcol = 0;
        break;

  ...
  for (i=0;i<lastop;i++) {
    ...
    switch (op[i].opcode)
    {
      case CONSTANTS:
        break;
      case SPHERE:
        if (op[i].op.sphere.constants != NULL) reflect = op[i].op.sphere.constants->s.c;
        else reflect = &white;
        ...
        break;
      case TORUS:
        if (op[i].op.torus.constants != NULL) reflect = op[i].op.torus.constants->s.c;
        else reflect = &white;
        ...
        break;
      case BOX:
        if (op[i].op.box.constants != NULL) reflect = op[i].op.box.constants->s.c;
        else reflect = &white;
      ...
      case ROTATE:
        ...
        if (op[i].op.rotate.axis == 1) tmp = make_rotY(theta);
        if (op[i].op.rotate.axis == 2) tmp = make_rotZ(theta);
        matrix_mult(peek(systems),tmp);
        copy_matrix(tmp,peek(systems));
        tmp->lastcol = 0;
        break;
